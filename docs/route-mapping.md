...
「网关登录校验」为 **是** 的服务由网关 ForwardAuth 统一校验 access token（JWT 验签 + 登出黑名单，见 [deploy/k8s/gateway/README.md](../deploy/k8s/gateway/README.md#登录校验forwardauth)），校验通过后网关向后端注入 `X-User-Id`/`X-Username` 请求头。
...
4. 需要用户身份时从网关注入的 `X-User-Id`/`X-Username` 请求头读取，不要在服务内解析 JWT
