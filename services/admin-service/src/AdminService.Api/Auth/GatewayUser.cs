namespace AdminService.Api.Auth;

/// <summary>
/// 经网关 ForwardAuth 登录校验后注入的用户身份与角色（deploy/k8s/gateway/auth-middleware.yaml）。
/// 网关会先删除客户端自带的同名头再用 sso-service 校验结果覆盖写入，
/// 因此这些头仅在请求经网关转发时可信；集群内直连或本地调试时为空。
/// </summary>
public sealed record GatewayUser(ulong UserId, string Username, IReadOnlyCollection<string> Roles)
{
    ...
    public static GatewayUser? FromRequest(HttpRequest request)
    {
        ...
        return new GatewayUser(userId, username, roles);
    }
}
